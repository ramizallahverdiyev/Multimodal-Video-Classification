...
- Large artifacts are ignored via `.gitignore`: `data/raw`, `data/interim`, `data/processed`, `results`, `.venv`. Use Git LFS if you must keep binaries/models.
- Keep API keys out of commits; rely on local `.env` or config copies.
- Share trained weights via Releases or LFS (`results/models`).

Adapt the configs to your scenario and start experimenting. The scaffold is minimal so you can iterate fast on multimodal training.***
